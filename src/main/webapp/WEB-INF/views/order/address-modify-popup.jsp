...
<main id="main" class="margin-top-60 margin-bottom-60 mobile-padding-rl-15 user">
	<div class="main-container">
		<div class="bright-back-color w-80p">
			<div class="p-20px" id="">
				<h2 class="fs-33 fw-bold margin-bottom-20">배송지 주소 수정</h2>
				...
				<div class="row m-row">
					<div class="col generic-input-box width90">
						<input type="tel" id="aiCountryCode" name="aiCountryCode" class="generic-input required-input zero-margin" required value="+82" maxlength="5">
						<label class="generic-label" for="aiCountryCode">국가 코드 <span aria-hidden="true">*</span></label>
					</div>
					<div class="col generic-input-box">
						<input type="tel" id="aiPhone" name="aiPhone" class="generic-input required-input zero-margin" required>
				...
				<div class="row mt-40" id="aiDefaultWrapper">
				...
				<input type="button" id="save" name="save" class="button-base button-primary height-normal mb-25 mt-40" value="저장 및 수정" onclick="save()">
			</div>
		</div>
	</div><!-- .main-container -->
</main>
...
async function getData(){
	let params = {
			uiNum : Number(userObject.uiNum)
	}
	console.log(params);
	console.log("성공3");
	axios.post('/address4',params)
	.then(res=>{
		address = res.data;
		aiNum = res.data.aiNum;
		console.log(aiNum);
		console.log("성공4");
		for (const key in address) {
			if(address[key]){
				...
				if(obj){
					if(obj.getAttribute('data-format')){
						const funcName = obj.getAttribute('data-format');
						window[funcName](obj,address[key]);
					}else{
...
async function save(){
	...
	.then(res =>{
		if(res.data>0){
			...
			alert("주소 수정에 실패하였습니다. 고객센터에 연락 부탁드립니다.");
		}
	});
}
</script>
