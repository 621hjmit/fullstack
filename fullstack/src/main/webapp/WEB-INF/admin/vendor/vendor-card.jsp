		...
		<div class="col-sm-12">
			<div class="card table-card">
				<div class="card-body p-4 ">
					...
					<div id="cardComGrid" style="width: 100%; height: 50vh;">
					</div>
				</div>
			</div>
		</div>
			...
			<div class="card-body p-4 text-center">
				<div class="btn-group" role="group">
					<div class="text-center">
						...
						<button onclick="deleteCardCom()" type="button" class="btn btn-danger">삭제</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	function init(){
		const columns = [
			...
			{width:55, id:'cciNum', header:[{text:'번호'}], editable:false},
			{width:90, id:'cciName', header:[{text:'카드사명'}]},
			{width:200,id:'cciId', header:[{text:'CODE'}]},
			{width:80, id:'cciContact', header:[{text:'담당자'}]},
			{width:150,id:'cciPhone', header:[{text:'연락처'}]},
			{width:250,id:'cciEmail', header:[{text:'이메일'}]},
			{width:100,id:'cciStartDate', header:[{text:'계약시작일'}]},
			{width:100,id:'cciEndDate', header:[{text:'계약만료일'}]},
		];
		grid = new dhx.Grid('cardComGrid',{
			columns : columns,
			leftSplit :1,
			adjust: true,
			editable: true,
	...
	async function getCardComs(){
		const res = await axios.get('/cardComs');
	...
	function modify(){
		...
		let result;
		axios.post('/cardComs', checkedRows)
		.then(res=>{
			result = res.data;
	...
	function deleteCardCom(){
		const rows = grid.data.serialize();
		const checkedRows = rows.filter(row=>row.rowId);
		const cciNums = rows.filter(row=>row.rowId).map(row=>row.cciNum);
		axios.delete('/cardComs',{data:cciNums})
		.then(res=>{
			result = res.data;
